...
const AddressForm = ({
  ...
  const inputStyle =
    "w-full appearance-none outline-none border border-gray-300 rounded-xs px-2 py-1 focus:border-1 focus:border-gray-400 focus:shadow text-md bg-white";
  ...
  const handleAddAddress = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post<AddAddressRespone>(
        `${BACKEND_URL}/user/addAddress`,
        formData,
  ...
  const handleUpdateAddress = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.patch<UpdateAddressRespone>(
        `${BACKEND_URL}/user/updateAddress`,
        formData,
        { withCredentials: true }
      );
    ...
    <div className="mx-auto max-w-sm md:max-w-md lg:min-w-full rounded-sm bg-blue-100 p-3 lg:p-4 flex justify-start items-start gap-2">
      ...
      <div className="w-full flex flex-col justify-start items-start gap-3">
        <p className="text-blue-600 text-md font-medium">
          {mode === "create" ? "ADD A NEW ADDRESS" : "UPDATE ADDRESS"}
        </p>
        ...
        <form
          onSubmit={mode === "create" ? handleAddAddress : handleUpdateAddress}
        ...
        >
          ...
          <div className="w-full flex justify-between gap-2 h-11">
            <input
              ...
              placeholder="Pincode"
            />
            <input type="text" className={inputStyle} placeholder="Landmark*" />
          </div>
          ...
          <div className="w-full flex justify-start items-center gap-3">
            <button
              type="submit"
              className="text-sm md:text-md text-white font-semibold bg-orange-500  px-6 md:px-8 py-3 rounded-xs hover:scale-95 hover:cursor-pointer"
            >
              {mode === "create"
            ...
            <p
              className="tet-sm md:text-md text-blue-600 bg-transparent hover:cursor-pointer font-medium px-2 py-2"
              onClick={() => {
                setShowAddressForm(false);
              }}
            >
