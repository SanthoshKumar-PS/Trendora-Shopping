...
type SellerProductProps = {
  ...
  setLastWeekSalesReport: React.Dispatch<React.SetStateAction<GetLastWeekSalesType | null>>
}

export function SellerProductSales({ lastWeekSalesReport }: SellerProductProps) {
  // Prepare chart data
  ...
  const totalVisitors = useMemo(() => lastWeekSalesReport?.totalProductsSold ?? 0, [lastWeekSalesReport])

  return (
    <Card className="flex flex-col">
      <CardHeader className="flex flex-col justify-center items-center pb-0">
        <CardTitle>Last Week Sales</CardTitle>
        <CardDescription>{lastWeekSalesReport?.range || getLastWeekRange()}</CardDescription>
      </CardHeader>
